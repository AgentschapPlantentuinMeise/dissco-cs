...
export function announcementsRoutes(repository) {
    const app = new Hono();
    app.get('/active', async (c) => {
        ...
        const announcements = await repository.announcements.listActiveAnnouncements(siteId, targetType, targetType === 'project' ? targetProjectSlug : null);
    ...
    app.post('/', async (c) => {
        ...
        const payload = parseAnnouncementBody((await c.req.json().catch(() => null)));
        if (!payload) {
