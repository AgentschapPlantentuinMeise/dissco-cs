...
## DiSSCo CS API database
psql -v ON_ERROR_STOP=1 --username "$POSTGRES_USER" <<-EOSQL
DO \$\$
BEGIN
  RAISE NOTICE 'adding new user $POSTGRES_DISSCO_CS_USER (dissco-cs)';
  CREATE ROLE $POSTGRES_DISSCO_CS_USER LOGIN PASSWORD '$POSTGRES_DISSCO_CS_PASSWORD';
  GRANT ALL PRIVILEGES ON DATABASE $POSTGRES_DB TO $POSTGRES_DISSCO_CS_USER;

  EXCEPTION WHEN DUPLICATE_OBJECT THEN
  RAISE NOTICE 'not creating role $POSTGRES_DISSCO_CS_USER -- it already exists';
END
\$\$;

CREATE SCHEMA IF NOT EXISTS $POSTGRES_DISSCO_CS_SCHEMA AUTHORIZATION $POSTGRES_DISSCO_CS_USER;

ALTER ROLE $POSTGRES_DISSCO_CS_USER SET search_path TO $POSTGRES_DISSCO_CS_SCHEMA, public;
EOSQL
